...
app.stage.on('pointermove', event => {
   if (isDragging) {
      ...
      if (timeElapsed > 0) {
         velocity.x =
            ((newPosition.x - lastDragPosition.x) / timeElapsed) *
            16.67 *
            VELOCITY_DAMPING;
         velocity.y =
            ((newPosition.y - lastDragPosition.y) / timeElapsed) *
            16.67 *
            VELOCITY_DAMPING;
      }
...
function cameraAdjustment(targetX, targetY) {
   ...
   function animate() {
      ...
      const statusBarHeight =
         document.getElementById('statusBar').offsetHeight + 40;
      ...
      if (markWorldX < 0) {
         ...
         adjustments.x =
            window.innerWidth - (markWorldX + markSize) - MARK_PADDING;
         needsAdjustment = true;
      }
      ...
      if (markWorldY < statusBarHeight) {
         ...
         adjustments.y =
            window.innerHeight - (markWorldY + markSize) - MARK_PADDING;
         needsAdjustment = true;
      }
      ...
      for (let dir = -1; dir <= 1; dir += 2) {
         ...
         for (let i = 1; i < WINNING_LENGTH; i++) {
            ...
            if (cell?.player === player) {
               ...
               const existing = potentialWins.get(lineKey) || {
                  count: 0,
                  cells: [],
               };
...
function updateHoverCell(event) {
   ...
   hoverGraphics.drawRect(worldX, worldY, cellSize * scale, cellSize * scale);
...
function updateMomentum() {
   ...
   const mousePosition = app.renderer.plugins.interaction?.mouse?.global ||
      app.renderer.events?.pointer || {x: 0, y: 0};
...
function highlightLastMove(cellX, cellY, player) {
   ...
   highlight.drawRect(cellX * cellSize, cellY * cellSize, cellSize, cellSize);
   highlight.endFill();
...
function showOffscreenIndicator(cellX, cellY, player) {
   ...
   const text = new PIXI.Text(
      `Click to view ${player} at (${cellX}, ${cellY})`,
      {
         fontSize: 14,
         fill: player === 'X' ? '#ff6961' : '#a2bffe',
         align: 'center',
   ...
   function updateIndicator() {
      ...
      if (
         worldX >= 0 &&
         worldX + cellWorldSize <= window.innerWidth &&
         worldY >= statusBarHeight &&
         worldY + cellWorldSize <= window.innerHeight
      ) {
      ...
      let indicatorX =
         window.innerWidth / 2 +
         Math.cos(angle) * (window.innerWidth / 2 - padding);
      let indicatorY =
         window.innerHeight / 2 +
         Math.sin(angle) * (window.innerHeight / 2 - padding);
      ...
      indicatorY = Math.max(
         statusBarHeight + boxHeight + padding,
   ...
   container.on('pointerdown', event => {
      ...
      const targetX =
         -(cellX * cellSize * scale) +
         window.innerWidth / 2 -
         (cellSize * scale) / 2;
      const targetY =
         -(cellY * cellSize * scale) +
         window.innerHeight / 2 -
         (cellSize * scale) / 2;
...
function makeBotMove() {
   ...
   try {
      ...
      const marksArray = Array.from(placedMarks.entries()).map(
         ([key, value]) => ({
            key,
            player: value.player,
         })
      ...
      const potentialWinsArray = Array.from(potentialWins.entries()).map(
         ([key, data]) => ({
            key,
            count: data.count,
